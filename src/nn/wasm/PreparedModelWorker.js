...
import { product, removeDuplicates, isTensor, sizeOfTensorData } from '../utils';

// Exported functions in the worker script can return one of the following:
//  - undefined
//  - { retVal: val }
...
async function getNNOpsInstance() {
...
function prepare(model) {
  ...
  for (let i = 0; i < model._operands.length; ++i) {
    ...
    if (isTensor(operand.type)) {
      runtimeOperand.value = _allocateTensor(operand);
      runtimeOperand.runtimeshape = _allocateRuntimeShape(operand);
      _toDelete.tensorValue.push(runtimeOperand.value);
      _toDelete.tensorShape.push(runtimeOperand.runtimeshape);
    } else {
      runtimeOperand.value = operand.value;
    }
    _operands.push(runtimeOperand);
  }
}

async function execute(inputs, outputs) {
  inputs.forEach(input => {
    let operand = _operands[input.index];
    let buffer = input.buffer;
    _setTensorData(operand.type, operand.value, buffer);
  });
  ...
  outputs.forEach(output => {
    let operand = _operands[output.index];
    let buffer = output.buffer;
    _getTensorData(operand.type, operand.value, buffer);
  });

  
  const outputArrayBuffers = outputs.map(o => o.buffer.buffer);

  return {
    retVal: outputs,
    // multiple arrays may share a same ArrayBuffer
    transferList: removeDuplicates(outputArrayBuffers),
  }
}

function _executeOperation(operation) {
  const nn_ops = _nn_ops;
  ...
  switch(op) {
    ...
    case OperationCode.ATROUS_CONV_2D: {
      ...
      let im2colShape = _allocateRuntimeShape(operand);
      let im2colData = _allocateTensor(operand);
    ...
    case OperationCode.RESIZE_BILINEAR: {
      ...
      let outSizeShape = _allocateRuntimeShape(operand);
      let outSizeData = _allocateTensor(operand);
...
function _setTensorData(type, ptr, data) {
  const nn_ops = _nn_ops;
...
function _getTensorData(type, ptr, buffer) {
  const nn_ops = _nn_ops;
  let view;
  if (type === OperandCode.TENSOR_FLOAT32) {
...
function _allocateTensor(operand) {
  const nn_ops = _nn_ops;
  let byteLength = sizeOfTensorData(operand.type, operand.dimensions);
  let ptr = nn_ops._malloc(byteLength);
  if (operand.lifetime === OperandLifetime.CONSTANT_REFERENCE) {
    _setTensorData(operand.type, ptr, operand.value);
  }
  return ptr;
}

function _allocateRuntimeShape(operand) {
  const nn_ops = _nn_ops;
...
async function deleteAll() {
  _toDelete.tensorValue.forEach(tensorValue => {
    _nn_ops._free(tensorValue);
  });
  _toDelete.tensorShape.forEach(tensorShape => {
    tensorShape.delete();
  });
  _operands = [];
  _operations = [];
