...
describe('CTS', function() {
  ...
  it('check result for Fully connected float large relaxed example', async function() {
    // For 'Fully connected float large relaxed' example: examples
    let model = await nn.createModel(options);
    let operandIndex = 0;

    let op1_value = [1, 10, 100, 500, 1000];
    let op3_expect = [9832];
    ...
    model.setOperandValue(b0, new Float32Array([900]));
