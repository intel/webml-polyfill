...
describe('CTS', function() {
  ...
  it('check result for Depthwise conv2d v1_2 example-33', async function() {
    // For 'Depthwise conv2d v1_2' example: examples_quant_output_multiplier_gt_1
    let model = await nn.createModel(options);
    let operandIndex = 0;

    let op14_value = [129, 131, 141, 143, 133, 135, 145, 147, 137, 139, 149, 151];
    let op44_expect = [255, 58, 255, 87, 255, 74, 255, 119];

    let type11 = {type: nn.TENSOR_QUANT8_ASYMM, dimensions: [1, 3, 2, 2], scale: 1.0058823529411764, zeroPoint: 127};
    let type11_length = product(type11.dimensions);
    let type12 = {type: nn.TENSOR_QUANT8_ASYMM, dimensions: [1, 2, 2, 4], scale: 1.0058823529411764, zeroPoint: 128};
    let type12_length = product(type12.dimensions);
    let type13 = {type: nn.TENSOR_INT32, dimensions: [4], scale: 1.0117993079584775, zeroPoint: 0};
    let type13_length = product(type13.dimensions);
    let type14 = {type: nn.TENSOR_QUANT8_ASYMM, dimensions: [1, 2, 1, 4], scale: 1.0, zeroPoint: 127};
    ...
    model.setOperandValue(op24, new Uint8Array([129, 130, 131, 132, 119, 138, 117, 140, 133, 134, 135, 136, 141, 114, 143, 112]));
    model.setOperandValue(op34, new Int32Array([2, 4, 6, 8]));
    ...
    model.setOperandValue(param32, new Int32Array([2]));
    model.setOperandValue(param33, new Int32Array([0]));
    model.addOperation(nn.DEPTHWISE_CONV_2D, [op14, op24, op34, param29, param30, param31, param32, param33], [op44]);
    ...
    let op44_output = new Uint8Array(type14_length);
  ...
  it('check result for Depthwise conv2d v1_2 example-34', async function() {
    // For 'Depthwise conv2d v1_2' example: examples_quant_output_multiplier_gt_1_relaxed
    let model = await nn.createModel(options);
    let operandIndex = 0;
  ...
  it('check result for Depthwise conv2d v1_2 example-35', async function() {
    // For 'Depthwise conv2d v1_2' example: examples_quant_output_multiplier_gt_1_weight_as_input
    let model = await nn.createModel(options);
    let operandIndex = 0;

    let op14_value = [129, 131, 141, 143, 133, 135, 145, 147, 137, 139, 149, 151];
    let op24_value = [129, 130, 131, 132, 119, 138, 117, 140, 133, 134, 135, 136, 141, 114, 143, 112];
    let op34_value = [2, 4, 6, 8];
    let op44_expect = [255, 58, 255, 87, 255, 74, 255, 119];
    ...
    model.setOperandValue(op24, new Uint8Array(op24_value));
    model.setOperandValue(op34, new Int32Array(op34_value));
  ...
  it('check result for Depthwise conv2d v1_2 example-36', async function() {
    // For 'Depthwise conv2d v1_2' example: examples_quant_output_multiplier_gt_1_weight_as_input_relaxed
    let model = await nn.createModel(options);
    let operandIndex = 0;
