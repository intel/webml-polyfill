class PoseNet {
  constructor(modelArch, version, useAtrousConv, outputStride, inputShape, cacheMap, backend, prefer) {
  ...
  async compute(inputTensor, heatmapTensor, offsetTensor, displacementFwd, displacementBwd) {
  ...
  _calculateProgress(current, length) {
    ...
    if (progressBar !== null && progressContainer !== null) {
      ...
      $('.loading-page .counter h1').html(`${loadedSize.toFixed(0)}/${totalSize.toFixed(0)} ${percentComplete}%`);
    }
  }
